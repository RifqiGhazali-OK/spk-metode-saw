    ...
    <div class="page-title mb-2">
        <div class="row align-items-end">
            <div class="col-12 col-md-12">
                <h3 class="mb-0">Dashboard User</h3>
                <p class="text-subtitle text-muted small">Ringkasan data penilaian Anda (SPK SAW)</p>
            </div>
        </div>
    </div>
                ...
                <div class="card-body py-3 px-4">
                    <div class="d-flex align-items-center">
                        <div class="flex-shrink-0 me-3 bg-info bg-opacity-10 rounded-3 p-2">
                            <i class="bi bi-bar-chart-steps fs-5 text-info"></i>
                        </div>
                        <div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-transparent d-flex justify-content-between align-items-center py-3">
                    ...
                    <a href="<?= base_url('user/hasil') ?>" class="btn btn-sm btn-outline-primary rounded-pill">
                ...
                <div class="card-body pt-0">
                    <div class="list-group list-group-flush">
                        <div class="list-group-item d-flex align-items-center px-0 py-2">
                            <div class="bg-light-primary rounded-3 p-2 me-3">
                                <i class="bi bi-cpu-fill text-primary"></i>
                            </div>
                            <div>
                                <h6 class="mb-0">Metode</h6>
                                <small class="text-muted">Simple Additive Weighting (SAW)</small>
                            </div>
                        </div>
                        <div class="list-group-item d-flex align-items-center px-0 py-2">
                            <div class="bg-light-success rounded-3 p-2 me-3">
                                <i class="bi bi-trophy-fill text-success"></i>
                            </div>
                            <div>
                                <h6 class="mb-0">Nilai Tertinggi Anda</h6>
                                <small class="text-muted fw-medium"><?= number_format($top_nilai ?? 0, 6) ?></small>
                            </div>
                        </div>
                        <div class="list-group-item d-flex align-items-center px-0 py-2">
                            <div class="bg-light-warning rounded-3 p-2 me-3">
                                <i class="bi bi-calendar-check-fill text-warning"></i>
                            </div>
                            <div>
                                <h6 class="mb-0">Waktu Saat Ini</h6>
                                <small class="text-muted" id="liveClockUser"></small>
                            </div>
                        </div>
                        <div class="list-group-item d-flex align-items-center px-0 py-2">
                            <div class="bg-light-danger rounded-3 p-2 me-3">
                                <i class="bi bi-shield-fill-check text-danger"></i>
                            </div>
                            <div>
                                <h6 class="mb-0">Status</h6>
                                <small class="text-success fw-semibold">● Sistem Aktif</small>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-transparent d-flex justify-content-between align-items-center py-3">
                    ...
                    <a href="<?= base_url('user/hasil') ?>" class="btn btn-sm btn-primary rounded-pill">
                            ...
                            <tbody>
                                <?php if (!empty($hasil_ranking)): ?>
                                    <?php foreach ($hasil_ranking as $row): ?>
                                        <tr>
                                            <td><?php if ($row['ranking'] == 1): ?><span class="badge bg-warning text-dark px-3 py-2 rounded-pill"><i class="bi bi-trophy-fill me-1"></i> #1</span><?php elseif ($row['ranking'] <= 3): ?><span class="badge bg-light-primary text-primary px-3 py-2 rounded-pill">#<?= $row['ranking'] ?></span><?php else: ?><span class="badge bg-secondary bg-opacity-25 text-dark px-3 py-2 rounded-pill">#<?= $row['ranking'] ?></span><?php endif; ?></td>
                                            <td><span class="badge bg-light text-dark px-3 py-2 rounded-pill"><?= htmlspecialchars($row['kode']) ?></span></td>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    </div><span class="small fw-semibold"><?= number_format($row['nilai_akhir'], 4) ?></span>
                                                </div>
                                            </td>
                                            <td><span class="badge <?= $row['status'] == 'Layak' ? 'bg-success' : 'bg-danger' ?> px-3 py-2 rounded-pill"><?= $row['status'] ?></span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" class="text-center py-5"><i class="bi bi-inbox fs-1 text-muted d-block mb-2"></i>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateClock() {
            ...
            document.getElementById('liveClockUser').innerText = day + ' ' + month + ' ' + year + ', ' + hours + ':' + minutes + ':' + seconds;
        }
        ...
        if (labels.length === 0) {
            document.querySelector('#chart-saw').innerHTML = '<div class="text-center text-muted py-5">Tidak ada data untuk ditampilkan</div>';
            return;
        }
        var options = {
            series: [{
